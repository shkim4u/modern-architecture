...
1. 쿠버네테스 (Amazon EKS) 작업을 위한 Tooling
    ...
    * Helm 설치
2. ~~AWS CLI 업데이트~~
    * 현재 새롭게 생성되는 Cloud9 환경에는 기본적으로 AWS CLI Version 2가 설치되어 있으므로 수행할 필요가 없습니다.
3. AWS CDK 업그레이드
4. 기타 도구 설치 및 구성
...
```bash
cd ~/environment/
curl -fsSL https://raw.githubusercontent.com/shkim4u/modern-architecture/main/cloud9/cloud9.sh | bash
```
